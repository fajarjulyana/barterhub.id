...
class ProductForm(FlaskForm):
    ...
    images = FileField('Gambar Produk (Maksimal 10)', validators=[
...
class ChatMessageForm(FlaskForm):
    message = TextAreaField('Pesan', 
                           validators=[
                               DataRequired(message='Pesan tidak boleh kosong'),
                               Length(min=1, max=1000, message='Pesan harus antara 1-1000 karakter')
                           ],
                           render_kw={"rows": 3, "placeholder": "Tulis pesan Anda..."})

    def validate_message(self, field):
        if not field.data or not field.data.strip():
            raise ValidationError('Pesan tidak boleh kosong atau hanya berisi spasi')
...
class ProfileEditForm(FlaskForm):
    ...
    profile_picture = FileField('Foto Profil', validators=[
