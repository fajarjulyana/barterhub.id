...
def get_rooms():
    ...
    try:
        # Enhanced query with proper joins to avoid missing data
    ...
    for room in rooms:
        ...
        if room.user1_id == current_user.id:
            other_user = room.user2.full_name
            current_user_role = 'user1'  # Buyer biasanya
        else:
            other_user = room.user1.full_name  
...
def room(product_id):
    ...
    # If user is the product owner, show them existing chat rooms for this product
    if product.user_id == current_user.id:
        # Find all chat rooms for this product
        ...
        if not existing_rooms:
            ...
            # Redirect to the single existing chat room
            chat_room = existing_rooms[0]
        else:
            # If multiple rooms exist, show the most recent one or let user choose
            chat_room = ChatRoom.query.filter_by(product_id=product_id).order_by(ChatRoom.created_at.desc()).first()
    else:
        # Find or create chat room for buyer
...
def send_negotiation(room_id):
    ...
    for offer in offered_products:
        ...
        if product and product.user_id == current_user.id:
            offer_details.append(f"• {product.title} ({product.total_points} poin, {product.condition})")
            total_offered_points += product.total_points
    
    # Add requested products details
    request_details = []
    for request in requested_products:
        if request.get('name'):
            desc = f" - {request['description']}" if request.get('description') else ""
            request_details.append(f"• {request['name']}{desc}")

    # Construct full message
    full_message = base_message
    if offer_details:
        full_message += f"\n\n🎁 Yang saya tawarkan:\n" + "\n".join(offer_details)
        full_message += f"\n💰 Total poin: {total_offered_points}"
    ...
    message = ChatMessage(
        ...
        message_type='offer',
        offered_products_json=json.dumps(offered_products),
        requested_products_json=json.dumps(requested_products)
    )
...
def accept_offer(message_id):
    ...
    transaction = Transaction(
        ...
        notes=f'Deal terkonfirmasi dari penawaran chat message #{message_id}'
    )

    # Set chat agreements as both parties agreed
    ...
    for offer in offered_products:
        ...
        if product:
            transaction_offer = TransactionOffer(
                ...
                points=product.total_points
            )
            db.session.add(transaction_offer)

    # Add comprehensive system message
    offer_summary = "\n".join([f"• {Product.query.get(o['product_id']).title}" for o in offered_products if Product.query.get(o['product_id'])])
    request_summary = "\n".join([f"• {r['name']}" for r in requested_products if r.get('name')])
    
    system_message = ChatMessage(
        room_id=chat_room.id,
        sender_id=current_user.id,
        message=f'🎉 DEAL TERKONFIRMASI! Transaksi #{transaction.id} berhasil dibuat.\n\n'
               f'📦 Produk yang ditawarkan:\n{offer_summary}\n\n'
               f'❤️ Produk yang diminta:\n{request_summary}\n\n'
               f'✅ Kedua pihak telah menyetujui deal ini. Silakan lanjut ke tahap pengiriman!',
        message_type='system'
    )
