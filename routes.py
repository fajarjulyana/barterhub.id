...
def get_rooms():
    ...
    for room in rooms:
        ...
        last_msg_preview = last_message.message if last_message else 'Belum ada pesan'
    ...
    if transaction.seller_received_at and transaction.buyer_received_at:
        ...
        if chat_room:
            system_message = ChatMessage(
                room_id=chat_room.id,
                sender_id=current_user.id,
                message=f'🎉 Transaksi barter berhasil diselesaikan! Kedua belah pihak telah mengkonfirmasi penerimaan barang dengan kode yang benar. Terima kasih telah menggunakan BarterHub!',
                message_type='system'
            )
            db.session.add(system_message)
