...
def accept_offer(message_id):
    ...
    try:
        ...
        # Generate confirmation codes immediately when deal is confirmed
...
def detail(id):
    ...
    if request.method == 'POST' and tracking_form.validate_on_submit():
        ...
        if transaction.status == 'agreed' and (not transaction.seller_confirmation_code or not transaction.buyer_confirmation_code):
            transaction.generate_confirmation_codes()
...
def confirm_received(id):
    ...
    if current_user.id == transaction.seller_id:
        # Seller confirming receipt from buyer - menggunakan kode dari paket pembeli
            ...
            if confirmation_code == expected_code:
                ...
                if chat_room:
                    system_message = ChatMessage(
                        room_id=chat_room.id,
                        sender_id=current_user.id,
                        message=f'✅ {transaction.seller.full_name} telah mengkonfirmasi penerimaan barang dari {transaction.buyer.full_name} dengan kode yang benar!',
                        message_type='system'
                    )
                    db.session.add(system_message)
            else:
                flash(f'❌ Kode konfirmasi salah. Kode yang benar: {expected_code}. Periksa kembali kode pada label paket yang Anda terima.', 'error')
                return redirect(url_for('transactions.detail', id=id))
        else:
            flash('Anda sudah mengkonfirmasi penerimaan barang sebelumnya.', 'info')

    elif current_user.id == transaction.buyer_id:
        # Buyer confirming receipt from seller - menggunakan kode dari paket penjual
            ...
            if confirmation_code == expected_code:
                ...
                if chat_room:
                    system_message = ChatMessage(
                        room_id=chat_room.id,
                        sender_id=current_user.id,
                        message=f'✅ {transaction.buyer.full_name} telah mengkonfirmasi penerimaan barang dari {transaction.seller.full_name} dengan kode yang benar!',
                        message_type='system'
                    )
                    db.session.add(system_message)
            else:
