...
class FloatingChat {
    ...
    openFullChat() {
        if (this.activeChatRoomId) {
            console.log('Opening full chat for room:', this.activeChatRoomId);
            ...
            .catch(error => {
                ...
                this.showError(`Gagal membuka chat: ${error.message}`);
                
                // Fallback: try to get product ID from current chat data
                ...
                if (activeTitle && activeSubtitle) {
                    // Try to extract product info and create a basic chat URL
                    const productName = activeSubtitle.textContent;
                    if (productName && productName !== 'Produk') {
                        // As a last resort, navigate to products list
                        window.location.href = '/products/';
                    }
                }
            });
        } else {
            this.showError('Pilih chat room terlebih dahulu');
            console.log('No active chat room to open');
        }
    }

    showChatRoomsList() {
        // Enhance chat rooms list with open full chat buttons
        const container = document.getElementById('chatRoomsList');
        if (!container) return;

        this.loadChatRooms().then(() => {
            // Add open full chat buttons to existing rooms
            const roomItems = container.querySelectorAll('.chat-room-item');
            roomItems.forEach(item => {
                if (!item.querySelector('.open-full-chat-btn')) {
                    const openBtn = document.createElement('button');
                    openBtn.className = 'btn btn-warning btn-xs open-full-chat-btn ms-2';
                    openBtn.innerHTML = '<i class="fas fa-expand"></i>';
                    openBtn.title = 'Buka Chat Room Penuh';
                    openBtn.style.fontSize = '0.7rem';
                    openBtn.style.padding = '0.2rem 0.4rem';
                    
                    // Extract room info from onclick attribute
                    const onclickAttr = item.getAttribute('onclick');
                    if (onclickAttr) {
                        const match = onclickAttr.match(/openChatRoom\((\d+),/);
                        if (match) {
                            const roomId = match[1];
                            openBtn.onclick = (e) => {
                                e.stopPropagation();
                                // Get product_id from room info and navigate to chat room page
                    ...
                    if (timeElement) {
                        timeElement.parentNode.insertBefore(openBtn, timeElement.nextSibling);
                    }
                }
            });
        });
...
window.FloatingChat = {
    openChatForProduct: function(productId, sellerName) {
        ...
        // Navigate directly to chat room page
        window.location.href = `/chat/room/${productId}`;
    },
