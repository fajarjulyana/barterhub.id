...
class FloatingChat {
    ...
    updateChatMessages(messages) {
        ...
        container.innerHTML = recentMessages.map(msg => {
            ...
            const messageContent = msg.message && msg.message !== 'undefined' ? msg.message : 'Pesan tidak dapat ditampilkan';
    ...
    async sendQuickMessage() {
        ...
        if (!message || !this.activeChatRoomId) {
    ...
    async acceptOffer(messageId) {
        if (!confirm('Apakah Anda yakin menerima penawaran ini?')) return;
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                this.showNotification('Penawaran Diterima', 'Transaksi berhasil dibuat!');
            ...
            alert(`Gagal menerima penawaran: ${error.message}. Coba lagi.`);
        }
    }
    ...
    async declineOffer(messageId) {
        ...
        try {
            ...
            alert(`Gagal menolak penawaran: ${error.message}. Coba lagi.`);
        }
    }
    ...
    async submitOffer() {
        ...
        if (selectedProducts.length === 0) {
            alert('Pilih minimal satu produk untuk ditawarkan');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                alert('Gagal mengirim penawaran: ' + (result.error || 'Unknown error'));
            }
        } catch (error) {
...
// Note: These global functions are intended for use by other parts of the application,
// likely on product detail pages, to trigger the floating chat.
