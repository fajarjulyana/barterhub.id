...
class User(UserMixin, db.Model):
    ...
    sent_messages = db.relationship('ChatMessage', foreign_keys='ChatMessage.sender_id', lazy='dynamic')
...
class Transaction(db.Model):
    ...
    def generate_confirmation_codes(self):
        ...
        # Ensure both codes are different
        while self.seller_confirmation_code == self.buyer_confirmation_code:
            self.buyer_confirmation_code = ''.join(secrets.choice(alphabet) for _ in range(8))
